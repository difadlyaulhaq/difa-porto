<script>
  ...
  export let mouseForce = 20;
  export let cursorSize = 100;
  export let isViscous = false;
  export let viscous = 30;
  export let iterationsViscous = 32;
  export let iterationsPoisson = 32;
  export let dt = 0.014;
  export let BFECC = true;
  export let resolution = 0.5;
  export let isBounce = false;
  export let colors = ['#5227FF', '#FF9FFC', '#B19EEF'];
  export let style = "";
  export let className = '';
  export let autoDemo = true;
  export let autoSpeed = 0.5;
  export let autoIntensity = 2.2;
  export let takeoverDuration = 0.25;
  export let autoResumeDelay = 1000;
  export let autoRampDuration = 0.6;
...
`;
    ...
    const io = new IntersectionObserver(
      entries => {
        ...
        if (!webgl) return;
        if (isEntryVisible && !document.hidden) {
          webgl.start();
        } else {
          webgl.pause();
        }
...
<div bind:this={mountRef} class={`liquid-ether-container ${className || ''}`} style={style}>
    <slot />
</div>
